...
log('login', login.status === 200 && !!login.json.data?.access_token,
  `status ${login.status}, perms=${login.json.data?.permissions?.length}`);
const token = login.json.data?.access_token;
const refreshToken = login.json.data?.refresh_token;
if (!token) { console.error(JSON.stringify(login.json, null, 2)); process.exit(1); }
...
log('item lookup', lookup.status === 200 && lookup.json.data?.length >= 1,
  `found ${lookup.json.data?.length} item(s)`);

// ─── 2b. POS bootstrap ─────────────────────────────────────────────────────────
const boot = await call('GET', '/v1/pos/bootstrap', null, token);
log('pos bootstrap', boot.status === 200 && !!boot.json.data?.default_series_id,
...
log('item list', list.status === 200 && Array.isArray(list.json.data),
...
log('item detail + derived stock', detail.status === 200 && Array.isArray(stock),
...
const itemBody = {
  ...
  pricing: {
    mrp: '100.00', sale_price: '90.00', purchase_price: '60.00',
    tax_inclusive: false, min_sale_price: null, max_discount_pct: null,
  },
  flags: {
    track_inventory: true, is_service: false, is_batched: false,
    allow_negative_stock: false, has_variants: false,
  },
  status: 'active',
};
...
log('  idempotent replay', replay.status === 201 && sameId && replayed,
...
log('create invoice', invoice.status === 201 && !!inv?.invoice_no,
  inv ? `${inv.invoice_no}, grand ${inv.grand_total}` : `status ${invoice.status} ${JSON.stringify(invoice.json.error)}`);
...
const oversell = await call('POST', '/v1/invoices', {
  client_id: randomUUID(),
  series_id: seed.seriesId,
  branch_id: seed.branchId,
  invoice_date: '2026-05-31',
...
log('negative-stock rejected', oversell.status === 400 &&
  oversell.json.error?.code === 'BUSINESS_RULE_VIOLATION',
...
log('refresh token', refreshed.status === 200 && !!newAccess && newRefresh !== refreshToken,
...
const createCust = await call('POST', '/v1/customers', {
  client_id: custId,
...
log('create customer', createCust.status === 201, `status ${createCust.status}, code ${createCust.json.data?.customer_code}`);

// Credit sale 1 — item B (₹499 + 5% = ₹524), no payment → balance due 524, under limit
function creditSale() {
  return call('POST', '/v1/invoices', {
    client_id: randomUUID(),
    series_id: seed.seriesId,
    branch_id: seed.branchId,
    invoice_date: '2026-05-31',
...
log('credit sale 1 (under limit)', sale1.status === 201, `status ${sale1.status}, grand ${sale1.json.data?.grand_total}`);

const outstanding = await call('GET', `/v1/customers/${custId}/outstanding`, null, token);
log('  outstanding reflects balance', outstanding.json.data?.balance_due === '524.00',
...
log('credit sale 2 (over limit) rejected', sale2.status === 400 &&
  sale2.json.error?.code === 'BUSINESS_RULE_VIOLATION',
  `status ${sale2.status}, code ${sale2.json.error?.code}`);

const custLookup = await call('GET', '/v1/customers/lookup?q=Credit', null, token);
log('customer lookup', custLookup.status === 200 && custLookup.json.data?.length >= 1,
...
log('has open invoice to settle', !!openInv, `invoice ${openInv?.invoice_no}, due ${openInv?.balance_due}`);

const receipt = await call('POST', '/v1/payments', {
  client_id: randomUUID(),
...
log('record receipt', receipt.status === 201,
...
log('  balance cleared after receipt', after.json.data?.balance_due === '0.00',
...
log('masters: units', units.status === 200 && units.json.data?.length >= 1, `${units.json.data?.length} units`);
const taxRates = await call('GET', '/v1/tax-rates', null, token);
log('masters: tax-rates', taxRates.status === 200 && taxRates.json.data?.length >= 1, `${taxRates.json.data?.length} rates`);
const banks = await call('GET', '/v1/bank-accounts', null, token);
const cashAcct = banks.json.data?.find((a) => a.type === 'cash');
log('masters: bank-accounts', banks.status === 200 && !!cashAcct, `${banks.json.data?.length} accounts`);
...
const sale4 = await call('POST', '/v1/invoices', {
  client_id: randomUUID(),
  series_id: seed.seriesId,
  branch_id: seed.branchId,
  invoice_date: '2026-05-31',
...
const receipt2 = await call('POST', '/v1/payments', {
  client_id: randomUUID(),
  payment_date: '2026-05-31',
  direction: 'inbound',
  party_type: 'customer',
...
log('  cash balance increased', cashAfter && Number(cashAfter.current_balance) >= Number(due4),
  `balance ${cashAfter?.current_balance}`);

const voidRes = await call('POST', `/v1/payments/${receipt2.json.data?.id}/void`, { reason: 'e2e test void' }, token);
...
log('  cash balance reversed', cashVoided && Number(cashVoided.current_balance) === 0,
  `balance ${cashVoided?.current_balance}`);

const reopened = await call('GET', `/v1/customers/${cust2Id}/outstanding`, null, token);
log('  invoice reopened after void', Number(reopened.json.data?.balance_due) >= Number(due4),
...
const itemUpd = await call('PATCH', `/v1/items/${seed.items.itemAId}`,
  { pricing: { sale_price: '275.00' } }, token, { 'If-Match': String(itemVer) });
log('item PATCH (optimistic)', itemUpd.status === 200 && itemUpd.json.data?.row_version === itemVer + 1,
  `v${itemVer} → v${itemUpd.json.data?.row_version}`);

// Re-using the stale version must 409.
const stale = await call('PATCH', `/v1/items/${seed.items.itemAId}`,
  { pricing: { sale_price: '300.00' } }, token, { 'If-Match': String(itemVer) });
log('  stale If-Match rejected', stale.status === 409, `status ${stale.status}, code ${stale.json.error?.code}`);

const custDetail = await call('GET', `/v1/customers/${custId}`, null, token);
const custUpd = await call('PATCH', `/v1/customers/${custId}`,
  { credit_limit: '5000.00' }, token, { 'If-Match': String(custDetail.json.data?.row_version) });
...
const createVendor = await call('POST', '/v1/vendors', {
  client_id: vendorId,
  name: 'Acme Supplies',
  type: 'Business',
  phone: '9111111111',
...
const purchase = await call('POST', '/v1/purchase-invoices', {
  client_id: randomUUID(),
  branch_id: seed.branchId,
  vendor_id: vendorId,
...
log('create purchase', purchase.status === 201, `${purchase.json.data?.voucher_no}, grand ${purchase.json.data?.grand_total}`);
...
log('  moving-avg purchase_price updated', stockAfter.json.data?.purchase_price === expectedAvg,
...
log('  vendor payable updated', vendorDetail.json.data?.payable === purchase.json.data?.grand_total,
  `payable ${vendorDetail.json.data?.payable}`);

// Duplicate vendor invoice no. → rejected.
const dupPurchase = await call('POST', '/v1/purchase-invoices', {
  client_id: randomUUID(),
  branch_id: seed.branchId,
  vendor_id: vendorId,
  vendor_invoice_no: 'VINV-001',
...
log('  duplicate vendor invoice rejected', dupPurchase.status === 409,
  `status ${dupPurchase.status}, code ${dupPurchase.json.error?.code}`);

// ─── 14. Reports ─────────────────────────────────────────────────────────────
const salesRpt = await call('GET', '/v1/reports/sales/summary?date_from=2026-05-01&date_to=2026-05-31', null, token);
log('report: sales summary', salesRpt.status === 200 && Number(salesRpt.json.data?.totals?.count) >= 1,
...
log('report: GSTR-1', gstRpt.status === 200 && gstRpt.json.data?.hsn_summary?.length >= 1,
...
log('report: stock valuation', stockRpt.status === 200 && Number(stockRpt.json.data?.total_value) > 0,
...
log('report: receivables', recvRpt.status === 200 && !!recvRpt.json.data?.aging,
  `total due ${recvRpt.json.data?.total_receivable}`);

const payRpt = await call('GET', '/v1/reports/financial/payables', null, token);
log('report: payables', payRpt.status === 200 && Number(payRpt.json.data?.total_payable) >= 9440,
...
log('invoice list', invList.status === 200 && invList.json.data?.length >= 1, `${invList.json.data?.length} invoices`);

const invGet = await call('GET', `/v1/invoices/${inv.id}`, null, token);
log('invoice get by id', invGet.status === 200 && invGet.json.data?.lines?.length >= 1,
...
log('public verify (no auth)', verify.status === 200 && verify.json.data?.verified === true &&
...
log('  bad hash → not verified', verifyBad.json.data?.verified === false, `verified=${verifyBad.json.data?.verified}`);
...
const adj = await call('POST', '/v1/stock-adjustments', {
  client_id: randomUUID(),
...
const trf = await call('POST', '/v1/stock-transfers', {
  client_id: randomUUID(),
...
log('stock transfer (10 → Back Store)', trf.status === 201, `${trf.json.data?.transfer_no}, status ${trf.json.data?.status}`);
...
const destQty = Number(itemBDetail.json.data?.current_stock?.find((s) => s.location_id === seed.location2Id)?.qty ?? 0);
log('  destination has 10', destQty === 10, `dest qty ${destQty}`);

// Transfer to same location must be rejected (schema refine → 400).
const trfBad = await call('POST', '/v1/stock-transfers', {
  client_id: randomUUID(),
  transfer_date: '2026-05-31',
  from_location_id: seed.locationId,
...
log('stock ledger view', ledger.status === 200 && ledger.json.data?.entries?.length >= 3,
...
const creditNote = await call('POST', '/v1/credit-notes', {
  client_id: randomUUID(),
  branch_id: seed.branchId,
...
log('create credit note', creditNote.status === 201, `${creditNote.json.data?.credit_note_no}, grand ${creditNote.json.data?.grand_total}`);

const cnStockAfter = (await stockB());
log('  stock restored (+1)', cnStockAfter === cnStockBefore + 1, `${cnStockBefore} → ${cnStockAfter}`);

const outAfter = await call('GET', `/v1/customers/${custId}/outstanding`, null, token);
log('  receivable reduced by 524', Number(outBefore.json.data?.balance_due) - Number(outAfter.json.data?.balance_due) === 524,
...
log('  original invoice fully_returned', sale3After.json.data?.status === 'fully_returned',
  `status ${sale3After.json.data?.status}`);

// Over-return guard: returning again must be rejected.
const cnAgain = await call('POST', '/v1/credit-notes', {
  client_id: randomUUID(),
  branch_id: seed.branchId,
  credit_note_date: '2026-05-31',
  original_invoice_id: sale3Id,
  reason: 'damaged',
  refund_mode: 'adjust_ledger',
  lines: [{ item_id: cnLine?.item_id, original_line_id: cnLine?.id, qty: cnLine?.qty, unit_id: cnLine?.unit_id, rate: cnLine?.rate, tax_rate_id: cnLine?.tax_rate_id, location_id: cnLine?.location_id, restore_stock: true }],
}, token);
log('  over-return rejected', cnAgain.status === 400, `status ${cnAgain.status}, code ${cnAgain.json.error?.code}`);
