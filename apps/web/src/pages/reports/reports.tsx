...
type Tab = 'sales' | 'purchases' | 'manufacturing' | 'gst' | 'stock' | 'receivables';
...
function ManufacturingReport() {
  const [subTab, setSubTab] = React.useState<'summary' | 'products' | 'consumption'>('summary');
  const [from, setFrom] = React.useState(firstOfMonth());
  const [to, setTo] = React.useState(today());
  ...
  const { data: productsData, isLoading: isProductsLoading } = useQuery({
    queryKey: ['rpt-prod-items', from, to],
    queryFn: () =>
      api.get<{
        items: {
          item_id: string;
      ...
      }>(`/reports/production/by-item?date_from=${from}&date_to=${to}`),
    enabled: subTab === 'products',
  });

  const { data: consumptionData, isLoading: isConsumptionLoading } = useQuery({
    queryKey: ['rpt-prod-consumption', from, to],
    queryFn: () =>
      api.get<{
        materials: { item_id: string; name: string; qty: string; value: string }[];
      }>(`/reports/production/consumption?date_from=${from}&date_to=${to}`),
    enabled: subTab === 'consumption',
  });

  const downloadProductsCSV = () => {
    if (!productsData || !productsData.items.length) return;
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `production-by-item-${from}-to-${to}.csv`);
    link.click();
  };

  const downloadConsumptionCSV = () => {
    if (!consumptionData || !consumptionData.materials.length) return;
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `material-consumption-${from}-to-${to}.csv`);
    link.click();
  };
