...
async function main() {
  try {
    // Get Cocoglo org
    const orgs = await sql`SELECT id, name FROM organizations WHERE name = 'Cocoglo' LIMIT 1`;
    if (!orgs || orgs.length === 0) {
      console.error('Cocoglo organization not found');
      process.exit(1);
    }
    const org = orgs[0];
    ...
    console.log('\n✅ Setup complete in Cocoglo organization!');
