...
export const InvoiceLineInputSchema = z.object({
  ...
  unit_id: UuidSchema,
  rate: MoneySchema,
  discount_pct: z.string().regex(/^\d+(\.\d{1,2})?$/, 'Must be a valid percentage').default('0'),
...
export const CreateInvoiceInputSchema = z.object({
  ...
  invoice_discount_pct: z.string().regex(/^\d+(\.\d{1,2})?$/, 'Must be a valid percentage').default('0'),
